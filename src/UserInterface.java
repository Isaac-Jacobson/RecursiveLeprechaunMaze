...
public class UserInterface implements CallbackInterface {
	...
	public void sendStatus(SearchStatus eStatus, int row, int col) {
		...
		if (eStatus == SearchStatus.PATH_COMPLETE) {
			...
			JOptionPane.showMessageDialog(frame, "Congratulations, you found the pot of gold!", "Maze Success",
		...
		// Display failure dialog, and exit
			...
			Image image = icon.getImage().getScaledInstance(200, 200, Image.SCALE_SMOOTH);
			icon = new ImageIcon(image);
			JOptionPane.showMessageDialog(frame, "Sorry, you cannot find the pot of gold!", "Maze Failure",
					JOptionPane.WARNING_MESSAGE, icon);
	...
	private void updateBoard() {
		...
		for (int row = 0; row < maze.length; row++) {
			for (int col = 0; col < maze[row].length; col++) {
				int index = (row * maze[row].length) + col;
				...
				if (leprechaunRow == row && leprechaunCol == col) {
					...
					case 0:
						button.setIcon(new ImageIcon(treeone));
						break;
					case 1:
						button.setIcon(new ImageIcon(treetwo));
						break;
					case 2:
						button.setIcon(new ImageIcon(treethree));
						break;
					case 3:
						button.setIcon(new ImageIcon(treefour));
						break;
					}
				} else {
					button.setIcon(null);
